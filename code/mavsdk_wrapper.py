...
async def measure_depth_center() -> float:
    depth_npy = 'depth_with_coordinates.npy'
    if not os.path.exists(depth_npy):
        return 999.0  # assume large safe distance

    data = np.load(depth_npy)  # shape: (N, 3) => [x, y, depth]

    # Derive width and height from the max x/y in the data
    max_x = int(data[:,0].max())
    max_y = int(data[:,1].max())

    width = max_x + 1
    height = max_y + 1

    # Define a bounding box for the center region (e.g. 40%-60%)
    x_min = 0.4 * width
    x_max = 0.6 * width
    y_min = 0.4 * height
    y_max = 0.6 * height
    ...
    if len(sub) == 0:
        # if no pixels in that region, fallback
        return 999.0
...
async def measure_depth_regions() -> dict:
    """
    Returns a dict with min depth in three regions:
      {
        "center": float,
        "above": float,
        "below": float
    ...
    depth_npy = 'depth_with_coordinates.npy'
    if not os.path.exists(depth_npy):
        # No file => treat as "infinite safe distance"
        return {"center": 999.0, "above": 999.0, "below": 999.0}

    data = np.load(depth_npy)

    width = int(data[:,0].max()) + 1
    height = int(data[:,1].max()) + 1

    def min_depth_in_box(xr_min, xr_max, yr_min, yr_max):
        ...
        if len(sub) == 0:
            return 999.0
        return float(sub[:,2].min())

    # "Center" box = middle 20% in both X & Y
    center_depth = min_depth_in_box(0.4, 0.6, 0.4, 0.6)
    # "Above" box = same X but a band above in Y (e.g. 0.2–0.4)
    above_depth = min_depth_in_box(0.4, 0.6, 0.2, 0.4)
    # "Below" box = same X but a band below in Y (0.6–0.8)
    below_depth = min_depth_in_box(0.4, 0.6, 0.6, 0.8)
